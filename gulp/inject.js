...
gulp.task('inject', ['scripts', 'styles', 'injectAuth', 'inject404', 'copyVendorImages', 'injectAyniHtmlDependencies'], function () {
  ...
  return gulp.src(path.join(conf.paths.src, '/index.html'))
    ...
    .pipe($.inject(gulppath, { ignorePath: [conf.paths.src, path.join(conf.paths.tmp, '/serve')], starttag: '<!-- inject:authentication -->'}))            
    .pipe($.inject(configpath, { ignorePath: [conf.paths.src, path.join(conf.paths.tmp, '/serve')], starttag: '<!-- inject:configuration -->'}))                                   
    .pipe(wiredep(_.extend({}, conf.wiredep)))
    .pipe(gulp.dest(path.join(conf.paths.tmp, '/serve')));
});
gulp.task('injectAyniDependencies', function(){
  return gulp.src(
      [
        path.join(conf.paths.src, './../dependencies/oidc-client.js'),        
        path.join(conf.paths.src, './../dependencies/jquery.js'),
        path.join(conf.paths.src, './../ayniconfig.js'),
        ]
...
gulp.task('injectAyniHtmlDependencies', ['injectAyniDependencies', 
    ...
    var injectStyles = gulp.src([
      path.join(conf.paths.tmp, '/serve/app/welcome.css'),
      path.join('!' + conf.paths.tmp, '/serve/app/vendor.css')
    ], {read: false});
